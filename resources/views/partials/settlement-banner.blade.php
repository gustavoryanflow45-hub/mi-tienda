        ...
        <div style="border-radius:10px; padding:14px 18px; margin-bottom:20px; display:flex; align-items:center; gap:10px;
            ...
            <div style="flex:1;">
                ...
                <div style="font-size:.78rem; color:#4a5568; margin-top:2px;">
                    Ventas ${{ number_format($update->data['total_sales'] ?? 0, 2) }}
                    · Comisión -${{ number_format($update->data['commission'] ?? 0, 2) }}
                    · Acreditado <strong>${{ number_format($update->data['net_amount'] ?? 0, 2) }}</strong>
