...
@section('title', __('Programa de afiliados'))
...
<div style="background:#f2f3f8; padding:30px 0 60px; min-height:60vh;">
    <div class="container" style="max-width:800px;">
        ...
        <div style="background:linear-gradient(135deg,#679941,#4e7a2e); border-radius:12px; padding:40px; color:#fff; text-align:center; margin-bottom:24px;">
            ...
            <h1 style="font-size:1.5rem; font-weight:700; margin-bottom:10px;">{{ __('Programa de afiliados') }}</h1>
            <p style="opacity:.9; font-size:.95rem;">{{ __('Gana comisiones recomendando nuestros productos a tus amigos y familia.') }}</p>
        </div>
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:16px; margin-bottom:24px;">
            <div style="background:#fff; border-radius:10px; box-shadow:0 1px 8px rgba(0,0,0,.07); padding:24px; text-align:center;">
                ...
                <h3 style="font-size:.95rem; font-weight:700; margin-bottom:8px;">1. {{ __('Comparte tu enlace') }}</h3>
                <p style="font-size:.83rem; color:#888;">{{ __('Registra tu cuenta y obtén tu enlace de referido único.') }}</p>
            </div>
            <div style="background:#fff; border-radius:10px; box-shadow:0 1px 8px rgba(0,0,0,.07); padding:24px; text-align:center;">
                <i class="las la-users" style="font-size:2rem; color:#679941; margin-bottom:12px; display:block;"></i>
                <h3 style="font-size:.95rem; font-weight:700; margin-bottom:8px;">2. {{ __('Atrae compradores') }}</h3>
                <p style="font-size:.83rem; color:#888;">{{ __('Cuando alguien compra usando tu enlace, ganas una comisión.') }}</p>
            </div>
            <div style="background:#fff; border-radius:10px; box-shadow:0 1px 8px rgba(0,0,0,.07); padding:24px; text-align:center;">
                <i class="las la-wallet" style="font-size:2rem; color:#679941; margin-bottom:12px; display:block;"></i>
                <h3 style="font-size:.95rem; font-weight:700; margin-bottom:8px;">3. {{ __('Cobra tus ganancias') }}</h3>
                <p style="font-size:.83rem; color:#888;">{{ __('Retira tus comisiones directo a tu cuenta bancaria.') }}</p>
            </div>
        </div>

        <div style="background:#fff; border-radius:10px; box-shadow:0 1px 8px rgba(0,0,0,.07); padding:28px; text-align:center;">
            @auth
                <p style="font-size:.9rem; color:#555; margin-bottom:16px;">{{ __('Tu enlace de referido:') }}</p>
            ...
            @else
                <p style="font-size:.9rem; color:#555; margin-bottom:16px;">{{ __('Regístrate para obtener tu enlace de afiliado.') }}</p>
